...
def generate_diet_plan(age: int, weight: float, height: float) -> str:
    """Ask Gemini to suggest a diet plan based on user profile"""
    prompt = f"""
    ...
    Please suggest a healthy daily diet plan including:
    - Breakfast
    - Lunch
    - Dinner
    - 2 snack options
...
llm_with_tools = llm.bind_tools([calculate_bmi, daily_calories, generate_diet_plan])
...
builder.add_node("tools", ToolNode([calculate_bmi, daily_calories, generate_diet_plan]))
